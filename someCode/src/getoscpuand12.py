...
#we  use getversion,verRes, add * between height,width.because the input raw file just have
#model, os_version,heitht, width, flag, idNums, launchNums
import sys
import re
...
#            print model1
    ...
    for line in fp2:
         ...
         key1 = tmp[0]+'\t'+tmp[1]+'\t'+tmp[2]+'\t'+tmp[3]#let model os_version height width be the key
...
#            print model
        ...
        s = os+ '\t'+ tmp[1]+ '\t'+ model+ '\t'+ cpu+'\t'+tmp[2]+'\t'+tmp[3].strip('\n')+'\t'+str(value[0]).strip('\n')+'\t'+str(value[1]).strip('\n')
