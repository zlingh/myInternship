 ...
 *  load logs *
 *************/
log = load 'out' as(os:chararray, model:chararray, cpu:chararray, resolution:chararray, modelNum:int, launchNum:int);
 ...
 * IOS logs     *
 ****************/
logIOS = filter log by not LOWER(os) matches 'android' and not LOWER(os) matches 'windowsphone'and not LOWER(os) matches 'unknow';
